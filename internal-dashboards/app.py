...
# Specify the credentials for your database
...
def get_data():
    ...
    session_start_df = sqlio.read_sql_query(
        'SELECT * FROM dlt_google_analytics_data.session_start', # Change this to your desired table name
        connection
    )

    # Daily values for the metrics page views, engaged sessions, and total sessions
    views_and_bounce_rate_df = sqlio.read_sql_query(
        'SELECT * FROM dlt_google_analytics_data.views_and_bounce_rate', # Change this to your desired table name
        connection
    )

    # Daily values for sessions by active users across the default channels  
    channel_split_df = sqlio.read_sql_query(
        'SELECT * FROM dlt_google_analytics_data.channel_split', # Change this to your desired table name
        connection
    )
